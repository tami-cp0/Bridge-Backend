...
export class InvestmentsService {
  ...
  async createInvestment(investorUserId: string, dto: CreateInvestmentDto) {
    ...
    await db.insert(notifications).values({
      ...
      body: `Your investment of ₦${dto.amountCommitted / 100} has been committed to the listing.`,
    });
  ...
  private async fundListing(listingId: string, listing: typeof listings.$inferSelect) {
    ...
    if (tranche1 && tranche1.status === 'locked') {
      ...
      await db.insert(notifications).values({
        ...
        body: `Your listing has been fully funded. Tranche 1 (₦${tranche1.amount / 100}) has been released to your account.`,
      });
    }
  }
}
