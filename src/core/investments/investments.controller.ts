...
export class InvestmentsController {
  ...
  @ApiResponse({
    status: 400,
    description:
      'Validation error, below minimum investment (â‚¦5,000), amount exceeds remaining unfunded amount, listing not active, or investor virtual account not found',
  })
  @ApiResponse({
    status: 401,
  ...
  @ApiOperation({
    summary:
      'Get all sweep events for a deal. If the caller invested in this listing, each event includes their distribution amount.',
  })
  @ApiResponse({
    status: 200,
    type: SweepEventWithDistributionDto,
    isArray: true,
  })
  @ApiResponse({
    status: 401,
