...
export class SweepController {
  ...
  @ApiOperation({ summary: 'Pay off the entire remaining balance on a listing in one transfer â€” releases any locked tranches first, then distributes to investors and closes the deal' })
  @ApiResponse({
    status: 201,
    schema: {
      type: 'object',
      properties: {
        ...
        message: { type: 'string', example: 'â‚¦32,250 repaid. Your listing is now completed.' },
      },
    },
  })
  ...
  @Post('squad')
  @HttpCode(200)
  @ApiOperation({ summary: 'Squad payment webhook â€” processes incoming virtual account payments and triggers revenue sweep' })
  ...
  @ApiResponse({ status: 401, description: 'Invalid webhook signature â€” x-squad-encrypted-body header present but HMAC verification failed' })
